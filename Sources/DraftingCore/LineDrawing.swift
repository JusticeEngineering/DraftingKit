...
public struct Rect2D: Sendable, Codable, Equatable {
    ...
    /// Creates a rectangle from its corners (not validated).
...
/// The library's output: classified 2D polylines in model units (y up),
/// plus tight bounds over every emitted point. Value type, Codable, and
/// canonically ordered — two equal inputs encode to identical bytes.
public struct LineDrawing: Sendable, Codable, Equatable {
    /// Whether a path is visible from the viewpoint or occluded behind
    /// geometry (drawn dashed by convention).
    public enum Kind: String, Sendable, Codable, CaseIterable {
        case visible
        case hidden
    }

    /// One polyline of a single kind.
    public struct Path: Sendable, Codable, Equatable {
        ...
        /// Creates a path (≥ 2 points expected).
    ...
    /// All polylines, canonically ordered (visible before hidden).
    public var paths: [Path]
    /// Tight bounds over every point in `paths`, in model units.
    /// `.zero` when there are no paths.
    public var bounds: Rect2D

    /// Creates a drawing from prebuilt parts; `bounds` is trusted as given.
